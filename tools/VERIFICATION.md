...
| wist4 `confirmation.json`, `derivation.json`, `coverage.json`, `extension.json`, `sanctions.json`, `superseded-audit.json`, `roster.json`, `link-agreement.json` | replay semantics recomputed by the generator's own logic | self-consistency | self-consistency-only |
